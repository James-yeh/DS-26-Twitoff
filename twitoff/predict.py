...
def predict_user(user0_name, user1_name, hypo_tweet_text):
    """
    Determines which user is more likely to say a given hypothetical text.

    Example run: predict_user('elonmusk', 'jackblack', 'Doge to the moon!')
    Returns a 0 for user0_name and a 1 for user1_name
    """
    ...
    # Create labels of len(vects)
    labels = np.concatenate(
        [np.zeros(len(user0.tweets)), np.ones(len(user1.tweets))])

    # Create and training our log_reg model
