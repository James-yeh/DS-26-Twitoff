...
# Creating a function to vectorize tweet
...
def add_or_update_user(username):
    ...
    Get user and get up to 200 of their tweets and add them to our
    SQLAlchemy DB.
    """
    try:
        twitter_user = api.get_user(username)
        
        # Decide whether or not to add or update
        ...
        for tweet in tweets:
            ...
            # Creating a tweet object to add to DB
            db_tweet = Tweet(id=tweet.id, text=tweet.full_text, vect=tweet_vector)
            # Connects the tweet to the user through tweets list (user.tweets)
            db_user.tweets.append(db_tweet)
            # If we add before appending we would likely get an error
